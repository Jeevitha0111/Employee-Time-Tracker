...
public class AdminDao {
    ...
    public boolean updateTask(Task task) {
        if (isTimeSlotAvailable(task)) {
            String sql = "UPDATE tasks SET employee_name = ?, role = ?, project = ?, date = ?, start_time = ?, end_time = ?, task_category = ?, description = ?, username = ?, password = ?, user_id = ? WHERE task_id = ?";
